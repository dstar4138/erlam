%%% A Multi-Core Bipartite-Graph Aided Queue Shuffling Scheduler
%%%
%%% This scheduler utilizes process cooperativity to attempt order the local
%%% process queue to be a optimal as possible.
%%% 
-module(erlam_sched_coop_ss).
...
options() ->
  ...
  "min_proc_threshold - The number of processes which would be allowed to resort.",
  "max_triggers - The number of triggers acceptable before triggering resort."
    }.
...
build_functionality( _Options ) -> % TODO: Only one currently.
    #state{preempt_fun=fun smartsort_preempts:default/2,
           steal_fun=fun smartsort_steals:default/1,
           spawn_fun=fun smartsort_spawns:default/2,
           yield_fun=fun smartsort_yields:default/3
          }.
